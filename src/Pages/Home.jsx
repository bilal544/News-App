...
const Home = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                setError(null);
                const response = await axios.get(`https://newsapi.org/v2/top-headlines?country=in&category=general&apiKey=${key}&pageSize=${pageSize}&page=${page}`, {
                    headers: {
    ...
    const handleNextClick = async () => {
        try {
            setLoading(true);
            setError(null);
            const response = await axios.get(`https://newsapi.org/v2/top-headlines?country=in&category=general&apiKey=${key}&pageSize=${pageSize}&page=${page + 1}`, {
                headers: {
                    'Content-Type': 'application/json'
                }
            });
        ...
        <>
            <main className='bg-black py-28 my-0'>
                <h1 className='text-center text-gray-100 font-openSans text-2xl font-bold leading-rela'>
                    NDTV &gt; <span className='text-red-600'>Home</span>
                </h1>
