...
const Entertainment = () => {
    ...
    useEffect(() => {
        ; (async () => {
            try {
                setLoading(true);
                setError(null);
                const response = await axios.get(`https://newsapi.org/v2/top-headlines?country=in&category=entertainment&apiKey=${key}&pageSize=${pageSize}&page=${page}`, {
                    headers: {
    ...
    const handleNextClick = async () => {
        try {
            setLoading(true);
            setError(null);
            const response = await axios.get(`https://newsapi.org/v2/top-headlines?country=in&category=entertainment&apiKey=${key}&pageSize=${pageSize}&page=${page + 1}`, {
                headers: {
                    'Content-Type': 'application/json'
                }
            });
        ...
        <>
            <main className='bg-black py-28 my-0'>
                <h1 className='text-center text-gray-100 font-openSans text-2xl font-bold leading-rela'>
                    NDTV &gt; <span className='text-red-600'>Entertainment</span>
                </h1>
            ...
            <main className='bg-gray-100 p-0 my-0'>
                <section className="container py-10 md:px-20 px-3">
                    ...
                    <div className="grid md:grid-cols-3 grid-cols-1 gap-4 w-auto">
                        {data.map((item, i) => (
